...
""",
    ...
    head_parser.add_argument(
        ...
        dest="lines",
        default=10,
        help="Number of lines to return. Ignored if byte_count is specified. Defaults to 10., defaults to 10",
    )
    head_parser.add_argument(
        "--mime-type", dest="mime_type", help="Return value as text/csv, text/markdown, or text/yaml inside the JSON."
    )
    ...
    head_markdown_parser.add_argument(
        "--lines", dest="lines", default=10, help="Number of lines to return. Defaults to 10., defaults to 10"
    )
    head_markdown_parser.add_argument(
        "--mime-type", dest="mime_type", help="Return value as text/csv, text/markdown, or text/yaml inside the JSON."
    )
    ...
    head_tail_parser.add_argument(
        ...
        dest="lines",
        default=10,
        help="Number of lines to read. Ignored if byte_count is specified. Defaults to 10., defaults to 10",
    )
    head_tail_parser.add_argument(
        "--mime-type", dest="mime_type", help="Return value as text/csv, text/markdown, or text/yaml inside the JSON."
    )
    head_tail_parser.add_argument(
        ...
        dest="mode",
        default="head",
        help="Operation mode, either 'head' or 'tail'. Defaults to 'head'., defaults to head",
    )
    ...
    cat_parser = subparsers.add_parser("cat", help="Mimics the basic functionalities of the 'cat' command in Unix..")
    ...
    cat_parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="If True, number all output lines., defaults to True",
    )
    cat_parser.add_argument(
        "--squeeze-blank",
        dest="squeeze_blank",
        action="store_true",
        default=False,
        help="If True, consecutive blank lines are squeezed to one., defaults to False",
    )
    ...
    cut_characters_parser.add_argument(
        "--character-ranges", dest="character_ranges", help='A string representing character ranges, e.g., "1-5,10".'
    )
    ...
    cut_fields_parser.add_argument(
        ...
        dest="delimiter",
        default=",",
        help="A single character used as the field delimiter., defaults to ,",
    )
    cut_fields_parser.add_argument(
    ...
    find_files_parser.add_argument("--file-type", dest="file_type", help="The type to filter ('file' or 'directory').")
    ...
    find_files_markdown_parser.add_argument("--name", dest="name", help="The exact name to match filenames against.")
    find_files_markdown_parser.add_argument(
    ...
    get_recent_commits_parser.add_argument(
        "--n", dest="n", default=10, help="The number of recent commits to retrieve. Defaults to 10., defaults to 10"
    )
    get_recent_commits_parser.add_argument(
        "--short-hash",
        dest="short_hash",
        action="store_true",
        default=False,
        help="If True, also return short hashes. Defaults to False., defaults to False",
    )
    ...
    git_log_file_parser = subparsers.add_parser("git_log_file", help="Returns the commit history for a specific file..")
    ...
    git_show_parser = subparsers.add_parser("git_show", help="Shows various types of objects (commits, tags, etc.)..")
    ...
    is_ignored_by_gitignore_parser.add_argument("--file-path", dest="file_path", help="The path of the file to check.")
    is_ignored_by_gitignore_parser.add_argument(
        "--gitignore-path",
        dest="gitignore_path",
        default=".gitignore",
        help="The path to the .gitignore file. Defaults to '.gitignore' in the current directory., defaults to .gitignore",
    )
    is_ignored_by_gitignore_parser.add_argument(
        "--mime-type", dest="mime_type", help="Return value as text/csv, text/markdown, or text/yaml inside the JSON."
    )
    ...
    apply_git_patch_parser = subparsers.add_parser(
        "apply_git_patch", help="Apply a unified (git) diff to the files in the root folder.."
    )
    apply_git_patch_parser.add_argument(
        "--mime-type", dest="mime_type", help="Return value as text/csv, text/markdown, or text/yaml inside the JSON."
    )
    apply_git_patch_parser.add_argument(
        "--patch-content", dest="patch_content", help="The content of the unified/git diff."
    )
    ...
    ls_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If True, include hidden files. Defaults to False., defaults to False",
    )
    ls_parser.add_argument(
        "--long",
        dest="long",
        action="store_true",
        default=False,
        help="If True, include details like permissions, owner, size, and modification date. Defaults to False., defaults to False",
    )
    ls_parser.add_argument(
        "--mime-type", dest="mime_type", help="Return value as text/csv, text/markdown, or text/yaml inside the JSON."
    )
    ...
    ls_markdown_parser.add_argument(
        ...
        dest="path",
        default=".",
        help="The directory path to list. Defaults to the current directory '.'., defaults to .",
    )
    ...
    grep_parser.add_argument(
        ...
        default=-1,
        help="Maximum number of matches to return for one file., defaults to -1",
    )
    grep_parser.add_argument(
        "--maximum-matches-total",
        dest="maximum_matches_total",
        default=-1,
        help="Maximum number of matches to return total., defaults to -1",
    )
    grep_parser.add_argument(
        "--mime-type", dest="mime_type", help="Return value as text/csv, text/markdown, or text/yaml inside the JSON."
    )
    ...
    grep_parser.add_argument(
        "--skip-first-matches",
        dest="skip_first_matches",
        default=-1,
        help="Number of initial matches to skip., defaults to -1",
    )
    ...
    grep_markdown_parser.add_argument(
        "--maximum-matches",
        dest="maximum_matches",
        default=-1,
        help="Maximum number of matches to return., defaults to -1",
    )
    grep_markdown_parser.add_argument(
        "--mime-type", dest="mime_type", help="Return value as text/csv, text/markdown, or text/yaml inside the JSON."
    )
    ...
    add_todo_parser.add_argument(
        ...
        dest="done_when",
        default="",
        help='Acceptance criteria — how to know the task is\ncomplete. Defaults to ""., defaults to ',
    )
    add_todo_parser.add_argument(
        "--mime-type", dest="mime_type", help="Return value as text/csv, text/markdown, or text/yaml inside the JSON."
    )
    ...
    query_todos_by_regex_parser = subparsers.add_parser(
        "query_todos_by_regex", help="Queries tasks by a keyword in their title, using a regular expression pattern.."
    )
    ...
    query_todos_by_regex_parser.add_argument(
        ...
        dest="regex_pattern",
        default="[\\s\\S]+",
        help='The regular expression pattern to match in task titles.\nDefaults to "[\\s\\S]+", which matches any title., defaults to [\\s\\S]+',
    )
    ...
    insert_text_at_start_or_end_parser.add_argument(
        ...
        dest="position",
        default="end",
        help="The position where the text should be inserted.\nShould be either 'start' or 'end'. Defaults to 'end'., defaults to end",
    )
    insert_text_at_start_or_end_parser.add_argument(
        "--text-to-insert", dest="text_to_insert", help="The text to insert into the file."
    )
    ...
    replace_line_by_line_parser.add_argument(
        ...
        dest="line_end",
        default=-1,
        help="The ending line number (0-indexed) for the replacement.\nIf -1, it goes to the end of the file. Defaults to -1., defaults to -1",
    )
    replace_line_by_line_parser.add_argument(
        "--line-start",
        dest="line_start",
    ...
    report_bool_parser.add_argument(
        "--comment", dest="comment", default="", help="Any comments, supplemental info about the answer., defaults to "
    )
    report_bool_parser.add_argument(
        "--mime-type", dest="mime_type", help="Return value as text/csv, text/markdown, or text/yaml inside the JSON."
    )
